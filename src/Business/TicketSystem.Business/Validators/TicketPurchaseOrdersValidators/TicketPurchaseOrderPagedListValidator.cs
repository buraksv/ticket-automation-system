using FluentValidation;
using TicketSystem.Common.Constants;
using TicketSystem.Dto.TicketPurchaseOrders;

namespace TicketSystem.Business.Validators;

internal sealed class TicketPurchaseOrderPagedListValidator : AbstractValidator<TicketPurchaseOrderSearchRequestDto>
{
    public TicketPurchaseOrderPagedListValidator()
    {
        RuleFor(x => x.Page).GreaterThan(0);
        RuleFor(x => x.PageSize).GreaterThan(0).LessThanOrEqualTo(ApplicationConstants.PagedListMaxPageSize);
    }
}